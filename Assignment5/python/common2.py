...
def convertToAngle(index,bins,min,max):
    return min + index*(max-min)/bins


def return_peak_values(H, window_size, threshold, xedges, yedges):
    peak_rows, peak_cols = extract_peaks(H, window_size, threshold)
    peak_theta = np.zeros((len(peak_rows)))
    peak_rho = np.zeros((len(peak_cols)))
    for i in range(len(peak_theta)):
        peak_theta[i] = xedges[peak_cols[i]]
        peak_rho[i] =  yedges[peak_rows[i]]
    return peak_theta, peak_rho